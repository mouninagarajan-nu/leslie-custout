...
- `api/` (Postgres connection + CORS): `DATABASE_URL` or the discrete `PGHOST`/`PGPORT`/`PGUSER`/`PGPASSWORD`/`PGDATABASE`, plus `DB_SSL`/`PGSSL`/`DATABASE_SSL` to toggle SSL (`rejectUnauthorized: false`), and `FRONTEND_ORIGIN` (must match the frontend's origin exactly for CORS to pass). See [api/.env.example](api/.env.example).
